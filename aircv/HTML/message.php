...
<body id="body" data-spy="scroll" data-target=".header">
    ...
    <h2 class="margin-b-0">Consultation</h2>

    <?php
    require 'connexion.php';
    $req = $conn->prepare("SELECT * FROM mbayem_message");
    $req->execute();
    $rows = $req->fetchAll();
    if (!$req) {
        ...
        <table>
            ...
            foreach ($rows as $key1 => $value) { ?>
                <tr>
                    ...
                    <?php for ($i = 0; $i < sizeof($value) / 2; $i++) { ?>
                        <td><?php echo $value[$i] ?></td>

                    <?php } ?>
                    <td><a href="suppression.php?tel=<?php echo $value[3] ?>" class="servic icon-close" > <br>Supprimer</a></td>
                </tr>
            <?php } ?>
        </table>
        <br>
